...
const Login = () => {
    ...
    // Mới thêm cho việc gọi API Login
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [error, setError] = useState('');
    const [isLoading, setIsLoading] = useState(false);

    // Mới thêm cho việc gọi API Register
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        if (regPassword !== regConfirm) {
            setRegError('Nhập lại mật khẩu không khớp!');
            return;
        }
        ...
        try {
            ...
            // Tự động chuyển tab sau 1.5s
            setTimeout(() => {
                setRegSuccess('');
                setEmail(regEmail); // Fill email automatically
                toggleForm('login');
                setIsLoading(false);
            }, 1500);
        } catch (err) {
                    ...
                    <div className="w-full max-w-[500px] relative">
                        ...
                        <div className="bg-white rounded-[2rem] shadow-2xl border border-white/60 overflow-hidden relative w-full z-10 transition-all duration-300"
                            ...
                            <div id="loginForm"
                                ...
                                {error && (
                                    <div className="mb-4 bg-rose-50 border border-rose-200 text-rose-600 px-4 py-3 rounded-xl text-sm font-medium text-center shadow-sm">
                                        {error}
                                    </div>
                                )}

                                <form id="login" onSubmit={handleLoginSubmit} className="space-y-4">
                                    {/* Email Field */}
                                    <div>
                                    ...
                                    <button type="submit" disabled={isLoading}
                                        className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3.5 px-4 rounded-2xl shadow-lg shadow-teal-600/30 hover:shadow-teal-700/40 transition-all focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 mt-2 flex items-center justify-center gap-2">
                                        {isLoading ? (
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-8">
                                    <button
                                        className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-full border border-slate-200 bg-white hover:bg-slate-50 focus:ring-2 focus:ring-slate-200 transition-colors text-sm font-medium text-slate-700">
                            ...
                            <div id="registerForm"
                                ...
                                {regSuccess && (
                                    <div className="mb-4 bg-teal-50 border border-teal-200 text-teal-700 px-4 py-3 rounded-xl text-sm font-medium text-center shadow-sm">
                                        {regSuccess}
                                    </div>
                                )}

                                <form id="register" onSubmit={handleRegisterSubmit} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label htmlFor="regConfirm" className="block text-xs font-semibold text-slate-700 mb-1.5 ml-1">Nhập lại</label>
                                    ...
                                    <div>
                                        <label htmlFor="regCode" className="block text-xs font-semibold text-slate-700 mb-1.5 ml-1">Mã xác nhận</label>
                                        <div className="flex gap-2">
                                            <input type="text" id="regCode"
                                                className="block w-full px-4 py-2.5 border border-slate-200 rounded-xl text-slate-900 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all outline-none text-sm"
                                                placeholder="Nhập mã 6 số" />
                                            <button type="button" className="whitespace-nowrap bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-semibold px-4 py-2.5 rounded-xl border border-slate-200 transition-colors focus:ring-2 focus:ring-teal-500 focus:outline-none">
                                                Gửi mã
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <button type="submit" disabled={isLoading}
                                        className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3.5 px-4 rounded-xl shadow-lg shadow-teal-600/30 hover:shadow-teal-700/40 transition-all focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 mt-4 flex justify-center items-center gap-2">
