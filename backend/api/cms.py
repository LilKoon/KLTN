...
def create_course(course: schemas.KhoaHocCreate, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
...
def get_courses(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
    return db.query(models.KhoaHoc).offset(skip).limit(limit).all()

@router.post("/lessons", response_model=schemas.BaiHocResponse)
def create_lesson(lesson: schemas.BaiHocCreate, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
...
def get_lessons_by_course(course_id: UUID, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
...
def create_question(question: schemas.NganHangCauHoiCreate, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
...
def get_questions(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db), _: models.NguoiDung = Depends(require_admin)):
